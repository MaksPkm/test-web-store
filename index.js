...
function createProduct(product) {
    return {
        ...
        imgSrc: product.imgSrc ? product.imgSrc : './images/goods/default.png',
    };
};
...
function renderGoods() {
    ...
    for (let i = 0; i < GOODS.length; i++) {
        ...
        const price = product.price === null ? 'empty' : `<p class="price">${product.price}</p>`
...
function renderCart() {
    ...
    for (let i = 0; i < goodsInCart.length; i++) {
        ...
        cartItem.innerHTML = `
            ...
            <div class="cart-item-count">${product.count} pcs</div>
...
function updateCartItem(id, count) {
    ...
    if (count > 1) {
        ...
        countElement.textContent = `${count -1} pcs`
    }
    else {
        cartItem.remove()
    }
